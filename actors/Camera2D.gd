...
#func _input(event):
#if event is InputEventMouseButton and not trauma:
#add_trauma(0.9)


func add_trauma(amount: float = 0.9):
...
func shake():
    ...
    offset.x = min(max_offset.x * amt * noise.get_noise_2d(noise.seed * 2, noise_y), max_offset.x)
    offset.y = min(max_offset.y * amt * noise.get_noise_2d(noise.seed * 3, noise_y), max_offset.y)
